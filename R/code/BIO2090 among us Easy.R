...
# Set up core data
Sprites <- data.frame(Names = c("Blackcurrant","Bubblegum","Burgundy","Cappuccino","Cocoa","Dalmation"),
                      chem1 = c(0),
                      chem2 = c(0),
                      chem3 = c(0),
                      prot1 = c(1),
...
victims <- c(0,0,0)
cluesize <- 3 # The final  clue is always the impostor sample so this needs to be greater than 1!
...
rmarkdown::render("code/Output standard.rmd", output_dir = "output", 
                  output_file = paste0("output/Output easy ",format(Sys.time(), "%y-%m-%d-%H-%M")))
