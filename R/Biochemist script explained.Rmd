...
```
## Overview
This script generates all the data and images required for a round of the "Among Us" style murder mystery. This script is appropriate for biochemists and requires a good understanding of the methods and class introduction document. The difficulty is set to "standard" - 18 suspects and a moderate level of errors introduced. This should be challenging for students who have not tried this before. Scripts with altered difficulty (number of suspects, level of errors) are provided too.
...
### Defined commands
Several modules are used repeatedly in the script. To reduce the length of the script, these are defined as commands in separate scripts that are placed into a folder labelled "code/Sub" under the desired main folder. The commands are defined in separate scripts to allow for individual replacement of these if necessary. Two of these are specific to the biochemist version.
...
```
The final parameters are to set up a variable for the "victims"; to define the number of clues to be given for each experiment each round (one of which is always the impostor; usually about half the total number of suspects); to define the error rate (as a percentage, with only whole numbers being meaningful); to set up a colour blind friendly colour palette; and to set up a data frame (using the tidyverse "tibble" version) for impostor data that will be reported in the final document.
...
The impostor result (which may now include an introduced error) is written to the impostor data table for later reporting. The impostor data will not be included in the gel so that more interpretation is required.
...
The "gel" command (from the "Gel" subroutine) is called. This requires arguments of the list of marker sizes (Markers), the matrix of information on the suspects and their proteins (gelsus, missing the final line with the impostor data), and the names of the sprites. The final command prints the image to the file selected.
The code for preparing the gels and western blots requires a good amount of calculation in this script, so these will tend to take 10-20 seconds each to run depending on computer speed.
```{r}
...
The western blot clue works very similarly to the gel. A list of suspects is assembled as above; information is placed in a matrix (here with just the name and the "glycosylation" group); the group can be randomised according to the error percentage. Either the "J" or "K" lectin is selected at random (this will be switched each round). 
Again, the impostor data is written to the impostor data table for reporting (which again may have an error).
An output image file is set, and the "west" command (defined in the "Western" subroutine) is called, with arguments of the marker size list, the matrix of data, and the selected probe. The image produced is then printed.
```{r}
...
Finally, the data are written to a specific data frame for round 1 for later reporting. The last line is not included so that the number of clues is consistent.
```{r}
...
The impostor data is written to the impostor data table for reporting (which may have errors).
...
## Round 2
The code for round 2 is very similar to round 1. The only changes are that the data saved for reporting are sent to new variables; the probe for the western blot is switched; and a different set of the HPLC possibilities from the impostor.
```{r}
...
```
## Round 3
The code for round 3 is identical to round 2, with the data saved for reporting are sent to new variables and another different combination of HPLC clues given.
```{r}
